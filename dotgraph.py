...
#given an edge, find the neighbors that can close the grid
def edge_neighbors(edge, rows, cols):
    r1, c1 = edge.v1.row, edge.v1.col
    r2, c2 = edge.v2.row, edge.v2.col
    if r1 == r2:
        #neighbors are upper and lower
        neighbors = [ Edge(Vertex(r1, c1), Vertex(r1+1, c1)),
                      Edge(Vertex(r1+1, c1), Vertex(r1+1, c2)),
...
#given a grid, find the neighbor grids and see if something can be closed
def edge_close(graph, edge, rows, cols):
    neighbors = edge_neighbors(edge, rows, cols)
    marked = {}
    unmarked = {}
    neighbor_map = { 'left' : neighbors[:3], 'right' : neighbors[3:] }
    for k, edges in neighbor_map.iteritems():
        marked[k] = filter(lambda edge: graph.is_connected(edge), edges)
        marked_edges = map(str, marked[k])
        print('Marked edges: %s' %marked_edges)
        unmarked[k] = filter(lambda edge: graph.is_connected(edge) == False, edges)
...
if __name__ == '__main__':
    ...
    num_grids -= num_grids_shared
    print('Num grids: %d' %num_grids)
    connections = get_all_edges(rows, cols)
    graph = Graph(edges = [])
    for edge in connections:
        graph.add(edge)
    edge = Edge( Vertex(1,0), Vertex(1, 1) )
    assert graph.is_connected(edge)
    #graph = Graph(edges = connections)
    v1 = Vertex(0, 0)
    v2 = Vertex(rows, cols)
    paths = graph.find_path(v1, v2)
    s_paths = map(str, paths)
    print('Path from Vertex: %s -> %s = %s' %( str(v1), str(v2), s_paths) )
    # vertices = [ ( Vertex(0, 0, 2), Vertex(0, 1, 2) ), ( Vertex(0, 1, 2), Vertex(0, 2, 1) ),
    #              ( Vertex(0, 0, 2), Vertex(1, 0, 3) ), ( Vertex(1, 0, 3), Vertex(1, 1, 2) ),
    #              ( Vertex(1, 1, 2), Vertex(1, 2, 1) ),
    # ]
    # g = Graph(connections = vertices)
    # paths = g.find_path( Vertex(0, 0), Vertex(0, 2) )
    # s_paths = map(str, paths)
    # print(s_paths)

    edge = Edge( Vertex(1, 0), Vertex(1, 1) )
    neighbor_edges = edge_neighbors(edge, rows, cols)
    s_edges = map(str, neighbor_edges)
    print('Edge: %s, Neighbors: %s' %( str(edge), s_edges) )
    edge = Edge( Vertex(3, 0), Vertex(3, 1) )
    neighbor_edges = edge_neighbors(edge, rows, cols)
    s_edges = map(str, neighbor_edges)
    print('Edge: %s, Neighbors: %s' %( str(edge), s_edges) )

    edge = Edge( Vertex(1, 1), Vertex(2, 1) )
    neighbor_edges = edge_neighbors(edge, rows, cols)
    s_edges = map(str, neighbor_edges)
    print('Edge: %s, Neighbors: %s' %( str(edge), s_edges) )
    unmarked_edge = edge_close(graph, edge, rows, cols)
    print('Unmarked edge for %s = %s' %(edge, unmarked_edge))
